...
import time
# Crank Nicholson for transient heat diffeq
...
if __name__ == "__main__":
    ...
    # plt.yscale('log')
    # plt.plot(grid_node_x, N[:,1], label="init_N")
    # plt.plot(grid_node_x, P[:,1], label="init_P")
    # plt.legend()
    ...
    for k in range(1,2):
        ...
        while(True):
            ...
            mat_A_N[0,0] = (dx / dt) + D_N * (-q / (2*kB*T) * (new_E_field[1]) - (1/dx))
            mat_A_P[0,0] = (dx / dt) + D_P * (q / (2*kB*T) * (new_E_field[1]) - (1/dx))
            mat_A_N[0,1] = D_N * (-q / (2*kB*T) * new_E_field[1] + (1/dx))
            mat_A_P[0,1] = D_P * (q / (2*kB*T) * new_E_field[1] + (1/dx))
            
            mat_A_N[m-1,m-1] = (dx / dt) - D_N * (-q / (2*kB*T) * (new_E_field[m-1]) + (1/dx))
            mat_A_P[m-1,m-1] = (dx / dt) - D_P * (q / (2*kB*T) * (new_E_field[m-1]) + (1/dx))
            ...
            mat_A_N[1:-1,1:-1] += np.diag((dx / dt) + D_N * (-q / (2*kB*T) * (np.roll(new_E_field, -1)[1:-2] - new_E_field[1:-2]) - (2/dx)), 0)
            mat_A_P[1:-1,1:-1] += np.diag((dx / dt) + D_P * (q / (2*kB*T) * (np.roll(new_E_field, -1)[1:-2] - new_E_field[1:-2]) - (2/dx)), 0)
            ...
            vec_B_N = N[:,k-1] * (dx/dt) + (rr + nrr) * dx
            vec_B_P = P[:,k-1] * (dx/dt) + (rr + nrr) * dx
            ...
            new_E_field[0] = E_field[0,k-1]
            new_E_field[m] = E_field[m,k-1]
            ...
            b2 = E_field[1:-1,k-1] + (q_C * dt / eps) * (D_P/dx * (new_P[1:] - np.roll(new_P, 1)[1:]) - D_N/dx * (new_N[1:] - np.roll(new_N, 1)[1:]))
                
            A2 = 1 - (q_C * dt / eps) * (D_P * (q / (2*kB*T)) * (new_P[1:] + np.roll(new_P, 1)[1:]) + D_N * (q / (2*kB*T)) * (new_N[1:] + np.roll(new_N, 1)[1:]))
    ...
    # plt.yscale('log')
    # plt.title('Drift/diffusion only, no source in control volume')
    # plt.legend()
    ...
    for k in range(2, n+1):
        ...
        while(True):
            ...
            mat_A_N[0,0] = 1.5 * (dx / dt) + D_N * (-q / (2*kB*T) * (new_E_field[1]) - (1/dx))
            mat_A_P[0,0] = 1.5 * (dx / dt) + D_P * (q / (2*kB*T) * (new_E_field[1]) - (1/dx))
            mat_A_N[0,1] = D_N * (-q / (2*kB*T) * new_E_field[1] + (1/dx))
            mat_A_P[0,1] = D_P * (q / (2*kB*T) * new_E_field[1] + (1/dx))
                        
            mat_A_N[m-1,m-1] = 1.5 * (dx / dt) - D_N * (-q / (2*kB*T) * (new_E_field[m-1]) + (1/dx))
            mat_A_P[m-1,m-1] = 1.5 * (dx / dt) - D_P * (q / (2*kB*T) * (new_E_field[m-1]) + (1/dx))
            mat_A_N[m-1,m-2] = -D_N * (-q / (2*kB*T) * new_E_field[m-1] - (1/dx))
            mat_A_P[m-1,m-2] = -D_P * (q / (2*kB*T) * new_E_field[m-1] - (1/dx))
            ...
            #     mat_A_N[i,i] = 1.5 * (dx / dt) + D_N * (-q / (2*kB*T) * (new_E_field[i+1] - new_E_field[i]) - (2/dx))
            #     mat_A_P[i,i] = 1.5 * (dx / dt) + D_P * (q / (2*kB*T) * (new_E_field[i+1] - new_E_field[i]) - (2/dx))
            ...
            mat_A_N[1:-1,1:-1] += np.diag(1.5 * (dx / dt) + D_N * (-q / (2*kB*T) * (np.roll(new_E_field, -1)[1:-2] - new_E_field[1:-2]) - (2/dx)), 0)
            mat_A_P[1:-1,1:-1] += np.diag(1.5 * (dx / dt) + D_P * (q / (2*kB*T) * (np.roll(new_E_field, -1)[1:-2] - new_E_field[1:-2]) - (2/dx)), 0)
            
            mat_A_N[1:-1,2:] += np.diag(D_N * (-q / (2*kB*T) * np.roll(new_E_field, -1)[1:-2] + (1/dx)))
            mat_A_P[1:-1,2:] += np.diag(D_P * (q / (2*kB*T) * np.roll(new_E_field, -1)[1:-2] + (1/dx)))
            
            
            # THese lag one iteration behind i.e. outer loop controlled
            f_0 = -sf * (new_N[0] * new_P[0] - n0 * p0) / (new_N[0] + new_P[0])
            f_L = -sb * (new_N[m-1] * new_P[m-1] - n0 * p0) / (new_N[m-1] + new_P[m-1])
            ...
            vec_B_N = (4 * N[:,k-1] - N[:,k-2]) * 0.5 * (dx/dt) + (rr + nrr) * dx
            vec_B_P = (4 * P[:,k-1] - P[:,k-2]) * 0.5 * (dx/dt) + (rr + nrr) * dx
            ...
            new_E_field[0] = 4*E_field[0,k-1] - E_field[0, k-2]
            new_E_field[m] = 4*E_field[m,k-1] - E_field[m, k-2]
            
            # for i in range(1,m):
            #     b = 0.5 * (4*E_field[i,k-1] - E_field[i, k-2]) + (q_C * dt / eps) * (D_P/dx * (new_P[i] - new_P[i-1]) - D_N/dx * (new_N[i] - new_N[i-1]))
                
            #     A = 1.5 - (q_C * dt / eps) * (D_P * (q / (2*kB*T)) * (new_P[i] + new_P[i-1]) + D_N * (q / (2*kB*T)) * (new_N[i] + new_N[i-1]))
            #     new_E_field[i] = (A ** -1) * b
                
            b2 = 0.5 * (4*E_field[1:-1,k-1] - E_field[1:-1, k-2]) + (q_C * dt / eps) * (D_P/dx * (new_P[1:] - np.roll(new_P, 1)[1:]) - D_N/dx * (new_N[1:] - np.roll(new_N, 1)[1:]))
                
            A2 = 1.5 - (q_C * dt / eps) * (D_P * (q / (2*kB*T)) * (new_P[1:] + np.roll(new_P, 1)[1:]) + D_N * (q / (2*kB*T)) * (new_N[1:] + np.roll(new_N, 1)[1:]))
            new_E_field[1:-1] = (A2 ** -1) * b2
